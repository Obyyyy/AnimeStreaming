<x-admin-layout>
    <section class="bg-white dark:bg-gray-900 border border-gray-200">
        <div class="py-8 mb-4` px-4 mx-auto max-w-2xl lg:py-16">
            <h2 class="mb-4 text-xl text-center font-bold text-gray-900 dark:text-white">Add new episode</h2>
            <form action="{{ route('admin.add.episodes') }}" method="POST" enctype="multipart/form-data">
                @csrf
                    ...
                    <div>
                        ...
                        <select id="anime" name="anime"
                            ...
                            @foreach ($animes as $anime)
                                <option value="{{ $anime->id }}" class="text-xs">{{ $anime->title }}</option>
                            @endforeach
                        </select>
                        {{-- <input type="text" name="course" id="course"
                            class="@error('course') is-invalid @enderror bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                            placeholder="Tuliskan judul materi" autocomplete="off"> --}}
                        @error('anime')
                            <span class="invalid-feedback" role="alert">
                                <strong class="text-xs text-red-600 mt-1">{{ $message }}</strong>
