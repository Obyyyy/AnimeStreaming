            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                <div
                    ...
                    <div class="w-full md:w-3/4 lg:w-3/4">
                        <form class="flex items-center relative" action="{{ route('admin.episodes') }}" method="GET">
                            {{-- @method('GET') --}}
                            <!-- Dropdown Select -->
                            <select id="anime" name="anime"
                                class="@error('anime') is-invalid @enderror bg-gray-50 border border-gray-300 text-gray-900 text-sm  rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-1/2 p-2 border-e-0 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                <option value="" class="text-xs" {{ request('anime') ? '' : 'selected' }}>Filter
                                    Anime</option>
                                @foreach ($animes as $anime)
                                    <option value="{{ $anime->slug }}" class="text-xs"
                                        {{ request('anime') == $anime->slug ? 'selected' : '' }}>
                                        {{ $anime->title }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div
                        ...
                        <a href="{{ route('admin.form.add.episodes') }}">
                            <button type="button"
                                class="flex items-center justify-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                <th scope="col" class="px-4 py-3">Anime Title</th>
                                <th scope="col" class="px-4 py-3">Episode</th>
                                <th scope="col" class="px-4 py-3">Video</th>
                                <th scope="col" class="px-4 py-3">Thumbnail Image</th>
                            ...
                            @foreach ($episodes as $index => $episode)
                                <tr class="border-b dark:border-gray-700">
                                    <th scope="row"
                                        ...
                                        {{ $episode->anime->title }}</th>
                                    <td class="px-4 py-3">{{ $episode->episode_name }}</td>
                                    <td class="px-4 py-3">{{ $episode->video }}</td>
                                    <td class="px-4 py-3">{{ $episode->thumbnail }}</td>
                                    <td class="px-4 py-3 flex items-center justify-end">
                                        ...
                                        <div id="dropdown-{{ $index }}"
                                            ...
                                            <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                                                ...
                                                <li>
                                                    <a href="{{ route('anime.watching', ['anime' => $episode->anime->slug, 'episode_name' => $episode->episode_name]) }}"
                                                ...
                                                <li>
                                                    <a href="{{ route('admin.form.edit.episodes', $episode->id) }}"
                                                        class="flex items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"><svg
                                                            class="w-5 h-5 text-gray-500 dark:text-gray-400"
                                                            aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                                            width="24" height="24" fill="none"
                                            ...
                                            <div class="py-1">
                                                <form id="delete-form-{{ $index }}"
                                                    action="{{ route('admin.delete.episodes', $episode->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
